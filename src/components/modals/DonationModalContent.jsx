export default function DonationModalContent({ profileInfo }) {
  return (
    <div className="space-y-4">
      <p className="text-cream/80 text-center">{profileInfo.donation.message}</p>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {/* GCash – left column */}
        <div className="border border-tan/15 rounded-lg p-4 bg-gradient-to-br from-tan/10 to-tan/5">
          <div className="flex items-center gap-3 mb-3">
            <div className="w-10 h-10 bg-cream/10 rounded-lg flex items-center justify-center shadow-sm">
              <svg className="w-6 h-6 text-green-500" fill="currentColor" viewBox="0 0 24 24">
                <path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-2 15l-5-5 1.41-1.41L10 14.17l7.59-7.59L19 8l-9 9z" />
              </svg>
            </div>
            <h3 className="text-lg font-semibold text-cream">GCash</h3>
          </div>
          <div className="space-y-2">
            <div className="flex items-center justify-between bg-cream/5 rounded-lg p-3">
              <span className="text-sm text-tan/60">Mobile Number:</span>
              <div className="flex items-center gap-2">
                <span className="font-mono font-semibold text-cream">{profileInfo.donation.gcash.number}</span>
                <button
                  onClick={() => {
                    navigator.clipboard.writeText(profileInfo.donation.gcash.number)
                    alert('GCash number copied to clipboard!')
                  }}
                  className="text-tan hover:text-cream cursor-pointer"
                  title="Copy number"
                >
            ...
            <div className="flex items-center justify-between bg-cream/5 rounded-lg p-3">
              <span className="text-sm text-tan/60">Account Name:</span>
              <span className="font-semibold text-cream">{profileInfo.donation.gcash.name}</span>
            </div>
            <a
        ...
        {/* PayPal – right column */}
        <div className="border border-tan/15 rounded-lg p-4">
          <div className="flex items-center gap-3 mb-3">
            <div className="w-10 h-10 bg-cream/10 rounded-lg flex items-center justify-center shadow-sm">
            ...
            <h3 className="text-lg font-semibold text-cream">PayPal</h3>
          </div>
          <div className="space-y-3">
            <div className="flex items-center justify-between bg-cream/5 rounded-lg p-3">
              <span className="text-sm text-tan/60">Email:</span>
              <span className="font-semibold text-cream">{profileInfo.donation.paypal.email}</span>
            </div>
            <p className="text-xs text-tan/60 mb-2">Quick amounts:</p>
            <div className="grid grid-cols-3 gap-2 mb-3">
              {profileInfo.donation.paypal.defaultAmounts.map((amount) => (
                <a
                  ...
                  className="px-3 py-2 rounded-lg border-2 border-tan/25 hover:border-tan/60 text-tan hover:text-cream transition cursor-pointer font-medium text-center text-sm"
                >
            ...
            <p className="text-xs text-tan/50 text-center">
